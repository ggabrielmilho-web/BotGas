...
Importa apenas o necessrio sem carregar todo o sistema
"""
import asyncio
import sys
import os
...
os.environ.setdefault("FINETUNED_EXTRACTOR_MODEL", "ft:gpt-4.1-mini-2025-04-14:carvalho-ia:botgas:CTt20bmy")
...
async def test_simple_message():
    ...
    assert result['product']['confidence'] > 0.6, \
        f" Esperado confiana > 0.6, recebido: {result['product']['confidence']}"
...
async def test_complete_message():
    ...
    # Validaes
    assert result['address']['street'] is not None, " Rua no foi extrada"
    assert result['address']['number'] == "45", \
        f" Esperado nmero 45, recebido: {result['address']['number']}"
...
async def run_all_tests():
    ...
    if failed == 0:
        ...
        print("\n MessageExtractor est funcionando corretamente!")
        print(" FASE 1 VALIDADA COM SUCESSO!")
    else:
        print(f"\n  {failed} teste(s) falharam")
        print("\n  Alguns testes podem falhar se o modelo fine-tuned")
        print("   ainda estiver em treinamento ou se a API key no estiver configurada.")
...
if __name__ == "__main__":
    ...
    if not os.getenv("OPENAI_API_KEY"):
        print("\n" + "="*60)
        print(" ERRO: OPENAI_API_KEY no est configurada!")
        print("="*60)
        print("\n Para rodar os testes, configure a varivel de ambiente:")
    ...
    try:
        ...
        print("\n\n  Testes interrompidos pelo usurio")
        sys.exit(1)
    except Exception as e:
